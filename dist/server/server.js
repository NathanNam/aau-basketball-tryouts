...
async function getStartManifest() {
  const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-DAS1ldpH.js");
...
function createStartHandler(cb) {
  ...
  const getEntries = async () => {
    if (routerEntry === null) {
      routerEntry = await import("./assets/router-ioNKvxMp.js").then((n) => n.r);
    }
    if (startEntry === null) {
...
const server$1 = createServerEntry({ fetch: fetch$1 });
const server = {
  fetch(request) {
